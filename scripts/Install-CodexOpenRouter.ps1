...
$script:ToolkitVersion = [version]'0.1.9'
...
function Get-ValidatedModulePackage {
    ...
    if ($RequireRelease) {
        ...
        if ([version]$manifest.ModuleVersion -ne $script:ToolkitVersion -or
            ...
            throw '待安装模块必须是 0.1.9，且只能导出 cx 与 cxor。'
        }
    }
...
if (-not $PSCmdlet.ShouldProcess(
        ...
        '安装 Codex OpenRouter Toolkit 0.1.9'
    )) {
    return
}
...
if ($legacyInfo -and (Test-Path -LiteralPath $legacyInfo.Root -PathType Container)) {
    ...
    try {
        ...
        Write-Warning "0.1.9 已安装，但旧版安装未能完全清理：$($_.Exception.Message)"
    }
}
