...
Security fixes target the current source version, `0.1.15`. See the [changelog](CHANGELOG.md) for release status. Users of older versions should update before checking whether an issue still occurs.
...
- Every OpenRouter Responses request first passes through the local cache-aware proxy. Claude requests without a custom cache policy receive only top-level `cache_control: {"type":"ephemeral"}` for upstream automatic caching with the default five-minute lifetime. The proxy adds no block-level breakpoints and preserves the original `input` structure, prompt text, roles, ordering, `instructions`, and tools. Claude requests with existing cache policies and other model requests retain their original bodies. Provider-side prompt caching temporarily retains prompt prefixes in provider infrastructure; retention, billing, and data-policy details vary by model and endpoint.
