...
安全修复面向当前源码版本 `0.1.13`。发布状态见 [变更记录](CHANGELOG.md)；旧版本用户应先更新，再确认问题是否仍然存在。
...
- 所有 OpenRouter Responses 请求先经过本机缓存感知代理。Claude 请求没有自定义缓存策略时，代理加入顶层默认 5 分钟 `ephemeral`，并为前部连续系统/开发者消息的首尾可标记消息添加最多两个缓存断点；提示文本、角色、顺序、`instructions` 与工具定义保留。已有缓存策略的 Claude 请求和其他模型保持请求正文不变。缓存会让提供商在其数据中心短期保留提示前缀，具体期限、计费与数据政策由模型和上游端点决定。
...
`cxor -CacheStatus` 只读访问当前代理的认证健康接口，不调用模型、不刷新目录、不启动或重启代理及桌面端。V5 代理对响应做有界内存解析并透传原始字节，健康信息仅保留累计计数、最近 token/费用数字和固定状态；不保存模型 ID、提示、响应正文或逐条请求历史。统计随代理退出而清空，不会写入状态文件。无法解析或缺失的用量保持未知，聚合统计无法证明整个系统提示命中缓存。
