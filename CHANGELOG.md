...
- `cxor` 默认执行原 `cxor -AllModels` 的行为，显示全部模型；`cxor -SetKey` 同样默认全量。
- 保留 `-AllModels` 作为兼容写法；`cxor -CacheStatus` 继续独立执行只读查询。

## 0.1.12 - 2026-09-23

- 修复上游移除 `~openai/gpt-latest` 后，有效新目录被旧默认入口校验拒绝、持续回退到旧目录的问题。
- 保留原精选入口，并在 GPT Latest 后加入 GPT Sol Latest（`~openai/gpt-sol-latest`）；默认模型按精选顺序选择目录中实际存在的入口，同步后将实际默认模型写入配置。
- 精选模式没有任何可用精选入口时拒绝候选目录，并提示使用 `cxor -AllModels`；全量模式仍优先选择精选入口，没有精选入口时使用首个非 `:batch` 模型。
- CLI 目录刷新使用的临时配置不再固定模型；旧目录回退警告补充脱敏、限长的失败原因。
- 同步中英文使用与安全说明，精选列表仅显示上游仍提供的入口，不再承诺固定模型数量。
