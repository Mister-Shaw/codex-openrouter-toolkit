...
- Claude 请求无自定义缓存策略时，仅加入顶层 `cache_control: {"type":"ephemeral"}`，适配 OpenRouter Responses API 支持的上游自动缓存，默认有效期仍为 5 分钟。
- 移除自动注入首尾 `prompt_cache_breakpoint` 的逻辑，保留 `input` 原结构；继续逐字节保留显式缓存策略，并保留稳定路由与缓存用量统计。
- 代理升级为 V6，使旧代理在下次启动时按现有校验流程轮换并加载新的自动缓存策略。

## 0.1.14 - 2026-09-23

- 保留配置中的多行字符串与数组边界，完整清理受管配置，避免误删字符串内的标记或留下失效配置片段。
- CLI 启动、超时和临时缓存读取失败分别记录原因并继续验证其他目录来源；密钥泄露仍会中止同步。
- 将 CLI 退出与输出读取纳入同一超时期限，防止继承输出管道的子进程使同步长期等待。
- 移除已不可达的精选过滤、重命名和排序逻辑；默认全量目录保留上游名称与顺序。
- 统一 Claude 请求解析、重写和路由键的 JSON 深度上限；缓存统计支持 CR、LF 与 CRLF 换行及跨块读取。
- 代理升级为 V5，使旧代理按现有校验流程轮换并加载本轮修复。
